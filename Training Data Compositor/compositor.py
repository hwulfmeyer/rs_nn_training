...
Original file is located at
    https://colab.research.google.com/drive/1qwZnt7xuTaAPeXumPyOwqJJe8MJWL23L
...
def generate_random_image(img, bg, edge_distance=0, rot=None, out_width=1600, withRandomMirroring=True):
    ...
    bg = copy.deepcopy(bg)
    scl = round(np.random.uniform(0.9, 1.10), 2)
    ...
    # random brightness
    brtn = round(np.random.uniform(0.9, 1.10), 2)
...
def secondstage(isTrainingData, saveImages=False, RotRepetitions=1, out_width=1600, withRandomMirroring=True):
    FOLDER = "validation"
    if isTrainingData:
        FOLDER = "training"
    OUT_PATH = "output/"
    ...
    for crop in tqdm(CROPS):
        for rot in range(1):
            for k in range(RotRepetitions):
                ...
                bg, img, pos, brtn, rot, scl = generate_random_image(img, bg, edge_distance=3, rot=rot, out_width=out_width, withRandomMirroring=withRandomMirroring)
...
"""def test(isTrainingData, color=None):
    FOLDER = "validation"
    if isTrainingData:
        FOLDER = "training"
    OUT_PATH = "output/"
    ...
    csv_rows = []
    tf_examples = []
    brtn_range = np.round(np.arange(0.85, 1.20, 0.05), 2)
    print(brtn_range)
    ...
    for k, crop in enumerate(CROPS):
      ...
      image_name = crop
    dst1.save(IMG_OUT_PATH+color+".png")"""

#test(isTrainingData=False, color='red')